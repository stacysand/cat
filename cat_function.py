...
# ROI coordinates
roi_table = (200, 100, 1000, 500)  # Region Of Interest
...
while True:  # runs forever until q
    ...
    detected_classes = [model.names[int(cls)] for cls in results_table[0].boxes.cls]
    if any(c in ('cat', 'dog') for c in detected_classes):
        trigger_function()

    # draw detection boxes and labels on the frame
    annotated_table = results_table[0].plot()  
