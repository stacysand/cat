...
# stream
stream = StreamReader(RTSP_URL).start()  # connect to the network stream
if not stream.cap.isOpened():
    print('Error: could not open the stream')
    sys.exit(1)  # sys.exit(1) used instead of exit() so systemd sees a real failure and restarts the service
print('Streaming')
...
# stuck-stream detection settings
STALE_TIMEOUT = 8     # seconds the picture is allowed to stay exactly the same before called "stuck"
MAX_RECONNECTS = 5    # consecutive failed reconnect attempts before give up and let systemd restart
 
# bookkeeping used to detect a stuck stream
last_frame_bytes = None          # the last frame actually processed
last_change_time = time.time()   # when the picture last actually changed
reconnect_attempts = 0           # consecutive failed reconnect attempts
 
 
def reconnect_stream():
    """NEW: tear down the current RTSP connection and open a brand new one from scratch."""
    global stream
    print("[main] Stream looks stuck — reconnecting...")
    stream.stop()
    new_stream = StreamReader(RTSP_URL).start()
...
while not shutdown:
    ...
    if not stream.running:
        ok = reconnect_stream()
        reconnect_attempts = 0 if ok else reconnect_attempts + 1
        if reconnect_attempts >= MAX_RECONNECTS:
            print("[main] Too many failed reconnects, exiting so systemd restarts us")
            sys.exit(1)
    ...
    # this is check that no stuck in the stream
    frame_bytes = frame.tobytes()
    if frame_bytes == last_frame_bytes:
        if time.time() - last_change_time > STALE_TIMEOUT:
            ok = reconnect_stream()
            reconnect_attempts = 0 if ok else reconnect_attempts + 1
            if reconnect_attempts >= MAX_RECONNECTS:
                print("[main] Too many failed reconnects, exiting so systemd restarts us")
            ...
            last_frame_bytes = None
            last_change_time = time.time()
            continue  # only skip here: just reconnected, grab a brand new frame first
    else:
